...
3. The Rust WebAssembly core owns LGIR semantics, diagnostics, hashing, normalization, migration, and target Markdown generation.
...
The checked-in JSON Schema is a portable editor contract. Rust remains the semantic authority because schema validation alone cannot enforce graph topology, structured loop ownership, target support, or deterministic ordering.
